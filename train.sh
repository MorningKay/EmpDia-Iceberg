...
DEBUGPY=${DEBUGPY:-0}
if [[ "$DEBUGPY" == "1" ]]; then
  PYBIN=(python -m debugpy --listen 127.0.0.1:5678 --wait-for-client -m)
else
  PYBIN=(python -m)
fi
...
# MODEL_PATH=${MODEL_PATH:-$PROJECT_DIR/models/Qwen2.5-0.5B-Instruct}
